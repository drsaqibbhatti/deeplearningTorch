...
class csp_residual_bottleneck_block(torch.nn.Module):

    def __init__(self, in_dim, mid_dim, out_dim, stride=1, part_ratio=0.5, activation=torch.nn.SiLU):
        super(csp_residual_bottleneck_block, self).__init__()
        ...
        self.residual_block = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=self.part2_chnls,
            ...
            torch.nn.BatchNorm2d(mid_dim),
            activation(),
            torch.nn.Conv2d(mid_dim, self.part2_out_chnls, kernel_size=1, stride=1, bias=False),
