######################################## Export PATH Before running code######################
# export PYTHONPATH=$PYTHONPATH:/home/saqib/deeplearningresearch/python
##############################################################################################
import torch
import random
import datetime
from datetime import date
from utils.ClassificationDataset import ClassificationDataset, generate_csv
...
def train(load_previous_model=False):
    ...
    # Hyperparameters
    imageWidth, imageHeight = 512, 512  # ImageNet image size
    batchSize = 768
    learningRate =  0.016* batchSize /4096
    epochs = 50
    targetAccuracy = 0.99999

    # Transformations
    transform = transforms.Compose([
    transforms.Resize((imageHeight, imageWidth), interpolation=Image.BILINEAR),  # Bilinear interpolation for smoother resizing
    transforms.ToTensor(),  # Converts image to tensor with values in the range [0, 1]
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # Normalize to ImageNet mean/std
    ])
    ...
    model = nfnetf0(class_num=num_classes)
    ...
    optimizer = torch.optim.RAdam(model.parameters(), lr=learningRate)

        # Wrap the optimizer with AGC
    agc_optimizer = AGC(
    ...
    # scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.029, max_lr=learningRate, step_size_up=TotalTrainBatch*2, mode='triangular')
    ...
    Base_dir = '/home/saqib/deeplearningtorch/Trained_Models/Classification/NFnet'
    model_name = "NFnet"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if train_acc > best_train_acc:
            ...
            # Save the scripted TorchScript model
            scripted_model = torch.jit.script(model.module if isinstance(model, DataParallel) else model)
